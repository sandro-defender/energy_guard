...
``energy_guard/config/dashboard``
    the aggregated dashboard home (protection totals, activity points,
    recent events, scan state and managed counts)
...
WS_DASHBOARD: Final = f"{WS_PREFIX}/dashboard"
...
@websocket_api.require_admin
@websocket_api.websocket_command({vol.Required("type"): WS_DASHBOARD})
@websocket_api.async_response
async def ws_dashboard(hass: HomeAssistant, connection, msg: dict[str, Any]) -> None:
    """Return the aggregated dashboard home of one entry (read-only)."""
    entry, error = _entry_or_error(hass, msg, msg.get("entry_id"))
    if error is not None:
        connection.send_error(*error)
        return
    ...
    connection.send_result(
        msg["id"],
        {"entry_id": entry.entry_id, "dashboard": build_dashboard(hub)},
    )
