...
| Home Assistant | `2026.9.x` (newest) and `2025.8.x` (verified minimum) | CI runs the suite against both ends of the range; each is pulled in by a pinned `pytest-homeassistant-custom-component` (`requirements_test.txt` / `requirements_test_min.txt`) |
